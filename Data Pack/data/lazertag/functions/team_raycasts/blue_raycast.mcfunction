...
execute as @e[tag=!raycasting,type=!#lazertag:not_mob,dx=0,team=!blue] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[dx=0] positioned ~0.99 ~0.99 ~0.99 run function lazertag:collide
scoreboard players set @s cooldown 15

scoreboard players add .distance tf_rc 1

# Make raycast visible
particle minecraft:dust 0 0.067 1 1 ~ ~ ~ 0 0 0 0 0
...
execute if score .distance tf_rc matches ..1000 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #lazertag:raycast_pass run function lazertag:team_raycasts/blue_raycast
